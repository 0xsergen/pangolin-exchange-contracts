...
contract PangolinRouter is IPangolinRouter {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = PangolinLibrary8.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = PangolinLibrary8.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint amountAOptimal = PangolinLibrary8.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                require(
                    ...
                    "PangolinRouter: INSUFFICIENT_OPTIMAL"
                );
                require(
    ...
    function removeLiquidityAVAX(
        ...
        returns (uint amountToken, uint amountAVAX)
    {
        ...
        unchecked {
            for (uint i; i < path.length - 1; ++i) {
                ...
                address to = i < path.length - 2
                    ? PangolinLibrary8.pairFor(factory, output, path[i + 2])
                    : _to;
                IPangolinPair(PangolinLibrary8.pairFor(factory, input, output))
        ...
        returns (uint[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            PangolinLibrary8.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
        ...
        unchecked {
            ...
            for (uint i; i < length - 1; ++i) {
                ...
                IPangolinPair pair = IPangolinPair(
                    PangolinLibrary8.pairFor(factory, input, output)
                );
                uint amountInput;
                uint amountOutput;
                {
                    ...
                    require(
                        pairBalance >= reserveInput,
                        "PangolinRouter: UNDERFLOW"
                    );
                    amountInput = pairBalance - reserveInput;
                    amountOutput = PangolinLibrary8.getAmountOut(
                        amountInput,
                        reserveInput,
                        reserveOutput
                    );
