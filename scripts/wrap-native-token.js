...
async function main() {
  ...
  const tokenAmount = ethers.utils.parseEther("50000"); // token amount to be wrapped
  ...
  // WRAPPING
  const txBytecode =
    wrappedNativeTokenContract.interface.encodeFunctionData("deposit");
  //   console.log(txBytecode);

  const txId = await multisigContract.transactionCount();
  ...
  console.log(`Tx for wrapping is sent to multisig. TxId is ${txId}`);
